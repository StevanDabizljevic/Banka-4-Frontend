export const getApiUrl = (route: string) => {
    ...
    if (isKoisnikRoute) {
        return 'http://banka-4-dev.si.raf.edu.rs/user-service/api'
    }
    const isBankaRoute = bankaRoutes.some(prefix => route.startsWith(prefix));
    if (isBankaRoute) {
        return 'http://banka-4-dev.si.raf.edu.rs/banka-service/api'
    }
    return 'http://banka-4-dev.si.raf.edu.rs/berza-service/api'
}